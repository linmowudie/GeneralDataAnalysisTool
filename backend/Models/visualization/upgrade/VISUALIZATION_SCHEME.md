...
1. 用户通过 [DataVisualization](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/dispatcher.py#L17-L108) 接口类传入参数：
...
2. [DataVisualization](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/dispatcher.py#L17-L108) 接口类根据模型名称确定任务类型，并构建参数字典

### 3.2 策略调度阶段

3. [Strategy](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/upgrade/strategy.py) 调度器根据任务类型自动选择最佳图表类型（如分类任务默认用混淆矩阵）

4. [VisualizationFactory](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/upgrade/visualization_factory.py) 根据模型名称和任务类型创建相应的可视化策略实例
...
6. 策略实例调用 [validate_params()](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/upgrade/base_visualization.py#L34-L37) 方法验证参数完整性
   - 检查必需参数是否存在
   - 验证数据格式是否正确
   - 确认模型对象是否包含必要的属性
...
7. 根据 [interactive](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/dispatcher.py#L42-L42) 参数决定调用 [generate_static_charts()](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/upgrade/base_visualization.py#L44-L51) 或 [generate_interactive_charts()](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/upgrade/base_visualization.py#L53-L60) 方法
...
每种策略都必须实现 [validate_params()](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/upgrade/base_visualization.py#L34-L37) 方法，确保：
- 必需参数存在
- 数据格式正确
- 模型对象包含必要属性
...
在底层绘图实现中，策略可以调用 [utils/](file:///f:/ProjectCode/GeneralDataAnalysisTool/backend/Models/visualization/upgrade/utils) 目录下的工具函数来复用常见的绘图逻辑，如绘制混淆矩阵、ROC曲线等。
