...
class AssociationRuleLearning(BaseAnalyzer):
    """
    关联规则学习模块
    专门处理关联规则任务的分析器
    """
    ...
    def __init__(
        ...
        # ===== 内部状态 =====
        self.task_type: str = 'association'
    ...
    def _initialize_model(self) -> None:
        """步骤4：用默认或用户传入参数初始化模型"""
        logger.info(f"初始化关联规则学习模型: {self.model_name}")
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("关联规则学习分析执行完成。")
            return result

        except Exception as e:
            logger.error(f"关联规则学习分析执行失败: {str(e)}")
            raise