...
class AgglomerativeClusteringAnalyzer(BaseAnalyzer):
    """
    凝聚聚类分析器
    """
    ...
    def load_params(self, config_path: str) -> Dict[str, Any]:
        ...
        # 提取预处理特殊参数
        if 'preprocessing_special_params' in full_config:
            model_params['preprocessing_special_params'] = full_config['preprocessing_special_params']
            
        # 提取模型特殊参数
        if 'ML_model_special_params' in full_config:
            # 查找与凝聚聚类相关的参数
            for model_type, params_dict in full_config['ML_model_special_params'].items():
                if isinstance(params_dict, dict) and ('agglomerative' in model_type.lower() or 'clustering' in model_type.lower()):
                    if 'ML_model_special_params' not in model_params:
                        model_params['ML_model_special_params'] = {}
                    model_params['ML_model_special_params'][model_type] = params_dict
    ...
    def validate_params(self, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.config and ('preprocessing_special_params' in self.config or 'ML_model_special_params' in self.config):
                ...
                # 检查ML_model_special_params中的凝聚聚类相关参数
            ...
            for param, value in params.items():
                if param in valid_params:
                    ...
                    if param == 'n_clusters' and value <= 0:
                        ...
                        return False
                    elif param == 'metric' and 'linkage' in params:
                        # 检查亲和度和连接方式的兼容性
                        if params['linkage'] in valid_metrics and value not in valid_metrics[params['linkage']]:
                            print(f"参数 {param} 值错误: 连接方式 {params['linkage']} 仅支持 {valid_metrics[params['linkage']]} 亲和度")
                            return False
    ...
    def train(self, X_train: pd.DataFrame, y_train: Optional[pd.Series] = None) -> Any:
        """
        训练凝聚聚类模型
        
        参数:
            X_train (pd.DataFrame): 训练特征数据
            y_train (Optional[pd.Series]): 目标数据（聚类任务中通常为空）
        ...
        try:
            ...
            if len(X_train) <= 500:  # 限制样本数量以避免过度计算
                self.linkage_matrix = linkage(X_train, method='ward')
            
            return self.model
        except Exception as e:
            raise Exception(f"凝聚聚类训练失败: {str(e)}")
    ...
    def postprocess(self, model: Any, X: pd.DataFrame, y: Optional[pd.Series] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算聚类评估指标
            if len(X) > len(np.unique(labels)) and len(X.columns) > 0:
                try:
            ...
            if self.linkage_matrix is not None and len(X) <= 100:  # 仅对小样本生成树状图
                try:
    ...
    def get_feature_importance(self, model: Any) -> Dict[str, float]:
        ...
        """
        try:
            # 由于凝聚聚类不直接提供聚类中心，我们计算每个簇的均值作为中心
        ...
        try:
            ...
            if hasattr(model, 'n_clusters'):
                new_model = AgglomerativeClustering(
                    ...
                    metric=getattr(model, 'metric', 'euclidean') if hasattr(model, 'metric') else 'euclidean',
                    linkage=getattr(model, 'linkage', 'ward') if hasattr(model, 'linkage') else 'ward'
                )
                # 注意：这里只是为了获取新数据的聚类，不是真正的预测
                # 实际上，凝聚聚类是不可预测的，这只是一个近似方法
    ...
    def analyzer(
        ...
        参数:
            ...
            model (str): 模型名称，如 "agglomerative"
        ...
        try:
            ...
            if 'linkage' not in params:
                params['linkage'] = 'ward'
            if 'metric' not in params and params.get('linkage') == 'ward':
            ...
            if len(X_train_scaled) <= 500:
                try:
                    ...
                    linkage_param = params.get('linkage', 'ward')
                    scipy_method = method_map.get(linkage_param, 'ward') if isinstance(linkage_param, str) else 'ward'
            ...
            # 计算聚类评估指标
            metrics = {}
            if len(X_train_scaled) > len(np.unique(train_labels)):
                try:
            ...
            # 添加树状图（如果样本数量合理）
            if self.linkage_matrix is not None and len(X_train_scaled) <= 100:
                try:
                    result["dendrogram"] = self._generate_dendrogram()
                except Exception as e:
                    print(f"生成树状图失败: {e}")
