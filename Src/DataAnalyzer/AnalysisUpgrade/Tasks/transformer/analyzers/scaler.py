...
class ScalerAnalyzer(BaseAnalyzer):
    """
    数据标准化分析器，支持StandardScaler和MinMaxScaler
    """
    ...
    def validate_params(self, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.config and ('preprocessing_special_params' in self.config or 'ML_model_special_params' in self.config):
                ...
                # 检查preprocessing_special_params中的scaler相关参数
                ...
                # 检查ML_model_special_params中的scaler相关参数
                if 'ML_model_special_params' in self.config:
                    for model_type, params_dict in self.config['ML_model_special_params'].items():
                        if isinstance(params_dict, dict) and ('standard_scaler' in model_type.lower() or 'minmax_scaler' in model_type.lower()):
    ...
    def train(self, X_train: pd.DataFrame, y_train: Optional[pd.Series]) -> Any:
        ...
        try:
            ...
            if self.scaler_type == "minmax":
                self.model = SklearnMinMaxScaler()
            else:
                self.model = SklearnStandardScaler()
                
            self.model.fit(X_train)
            return self.model
        except Exception as e:
            raise Exception(f"Scaler训练失败: {str(e)}")
    ...
    def predict(self, model: Any, X: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            # 返回标准化后的数组作为Series
            return pd.Series(X_scaled.flatten())
        except Exception as e:
            raise Exception(f"Scaler预测失败: {str(e)}")
    ...
    def analyzer(
        ...
        try:
            ...
            # 标准化数据
            scaled_data = self.predict(trained_model, X)
            
            return {
                "model": trained_model,
                "scaled_data": scaled_data,
                "scaler_type": self.scaler_type,
                "message": f"{self.scaler_type} Scaler训练和应用成功"
            }
        except Exception as e:
            return {
                "error": str(e)
