...
class RandomForestAnalyzer(BaseAnalyzer):
    ...
    def validate_params(self, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.config and ('preprocessing_special_params' in self.config or 'ML_model_special_params' in self.config):
                ...
                # 检查ML_model_special_params中的随机森林相关参数
                if 'ML_model_special_params' in self.config:
                    for model_type, params_dict in self.config['ML_model_special_params'].items():
                        if isinstance(params_dict, dict) and ('random_forest' in model_type.lower() or 'classification' in model_type.lower()):
            ...
            for param, value in params.items():
                if param in valid_params:
                    ...
                    if param == 'n_estimators' and value <= 0:
                        ...
                        return False
                    elif param == 'max_depth' and isinstance(value, int) and value <= 0:
                        print(f"参数 {param} 值错误: 必须大于 0")
                        return False
                        ...
                        print(f"参数 {param} 值错误: 必须大于等于 0")
                        return False
                    elif param == 'max_samples' and isinstance(value, (int, float)) and value <= 0:
                        print(f"参数 {param} 值错误: 必须大于 0")
                        return False
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series) -> Any:
        ...
        try:
            ...
            raise Exception(f"随机森林训练失败: {str(e)}")
