...
class RandomForestRegressorAnalyzer(BaseAnalyzer):
    ...
    def validate_params(self, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.config and ('preprocessing_special_params' in self.config or 'ML_model_special_params' in self.config):
                ...
                # 检查ML_model_special_params中的随机森林回归相关参数
                if 'ML_model_special_params' in self.config:
                    for model_type, params_dict in self.config['ML_model_special_params'].items():
                        if isinstance(params_dict, dict) and ('random_forest' in model_type.lower() or 'regressor' in model_type.lower()):
            ...
            # 降级方案：硬编码的有效参数列表（RandomForestRegressor）
            valid_params = {
                'n_estimators': int,
                'criterion': str,
                ...
                'warm_start': bool,
                'ccp_alpha': float,
                'max_samples': (int, float, type(None)),
                'monotonic_cst': (list, np.ndarray, type(None))
            }
            
            # 支持的评估标准
            valid_criteria = ['squared_error', 'absolute_error', 'friedman_mse', 'poisson']
            ...
            for param, value in params.items():
                if param in valid_params:
                    ...
                    # 检查值范围和有效值
                    if param == 'n_estimators' and value <= 0:
                        ...
                        return False
                    elif param == 'max_depth' and value is not None and value <= 0:
    ...
    def train(self, X_train: pd.DataFrame, y: Optional[pd.Series] = None) -> Any:
        ...
        try:
            ...
            raise Exception(f"随机森林回归训练失败: {str(e)}")
    ...
    def analyzer(
        ...
        参数:
            ...
            model (str): 模型名称，如 "random_forest_regressor"
        ...
        try:
            ...
            # 处理是否使用标准化
            use_scaling = model_params.pop('use_scaling', False)
            if use_scaling:
            ...
            result["n_trees"] = params.get('n_estimators', 100)
            result["max_depth"] = params.get('max_depth', 'None')
            
            # 如果启用了OOB评分，获取OOB评分
            if params.get('oob_score', False) and hasattr(self.model, 'oob_score_'):
                result["oob_score"] = float(self.model.oob_score_)
