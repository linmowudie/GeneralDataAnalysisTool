...
class PCAAnalyzer(BaseAnalyzer):
    ...
    def validate_params(self, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.config:
                # 从预处理方法参数中获取
                if 'preprocessing_special_params' in self.config and 'pca' in self.config['preprocessing_special_params']:
                    preprocess_config = self.config['preprocessing_special_params']['pca']
                ...
                if 'ML_model_special_params' in self.config:
                    ...
                    for task_type in self.config['ML_model_special_params'].values():
                        if isinstance(task_type, dict) and 'pca' in task_type:
                            pca_config = task_type['pca']
                            valid_params.update({p for p in pca_config.keys() if not p.startswith('key_')})
            
            # 如果配置未加载或没有找到参数，使用硬编码的默认参数列表作为降级方案
            if not valid_params:
                valid_params = {
                    'n_components', 'copy', 'whiten', 'svd_solver', 
                    'tol', 'iterated_power', 'random_state', 'n_oversamples', 
                    'power_iteration_normalizer', 'mat_solver'  
                }
            
            # 检查所有参数名是否在有效列表中
            for param_name in params:
                if param_name not in valid_params:
                    print(f"警告: PCA不支持的参数 '{param_name}'")
                    # 不抛出异常，允许用户尝试使用未知参数，但发出警告
            
            # 参数范围校验
            if 'n_components' in params and params['n_components'] is not None:
                if not isinstance(params['n_components'], (int, float)):
                    raise ValueError("n_components必须是整数或浮点数")
                if isinstance(params['n_components'], float) and not (0 < params['n_components'] <= 1):
                    raise ValueError("当n_components为浮点数时，必须在(0, 1]范围内")
                if isinstance(params['n_components'], int) and params['n_components'] <= 0:
                    raise ValueError("当n_components为整数时，必须大于0")
            
            if 'tol' in params and params['tol'] < 0:
                raise ValueError("tol必须大于等于0")
    ...
    def train(self, X_train: pd.DataFrame, params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        try:
            ...
            if params is None:
                params = {}
            self.model = SklearnPCA(**params)
            # PCA是无监督学习算法，不需要y_train参数
            self.model.fit(X_train)
            return self.model
        except Exception as e:
            raise Exception(f"PCA训练失败: {str(e)}")
    ...
    def get_feature_importance(self, model: Any) -> Dict[str, float]:
        ...
        """
        try:
            # PCA中各主成分的解释方差比可以看作重要性
            importance = {}
            for i, ratio in enumerate(model.explained_variance_ratio_):
                importance[f"PC{i+1}"] = float(ratio)
            return importance
        except Exception as e:
            # 发生异常时返回空字典而不是错误信息，因为函数签名要求返回Dict[str, float]
            return {}
    ...
    def predict(self, model: Any, X: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            raise Exception(f"PCA预测失败: {str(e)}")
    ...
    def analyzer(
        ...
        try:
            ...
            # 后处理获取解释方差比
            postprocess_result = self.postprocess(trained_model, X, None)
            
            # 准备返回结果
            result = {
                "model": trained_model,
                "reduced_data": reduced_data,
                "explained_variance_ratio": postprocess_result.get("explained_variance_ratio", []),
                "cumulative_variance_ratio": postprocess_result.get("cumulative_variance_ratio", []),
                "message": "PCA训练和应用成功"
            }
            ...
            if metrics_list or is_return_model_score:
                ...
                # 计算重构误差
                if "reconstruction_error" in calculate_metrics:
                    try:
                        # 执行降维以获取正确格式的数据用于重构
                        X_reduced_array = trained_model.transform(X)
                        # 重构原始数据
                        X_reconstructed = trained_model.inverse_transform(X_reduced_array)
                        # 计算MSE作为重构误差
                        reconstruction_error = np.mean((X.values - X_reconstructed) ** 2)
                        result["reconstruction_error"] = reconstruction_error
                    except Exception as re_error:
                        result["reconstruction_error_warning"] = f"无法计算重构误差: {str(re_error)}"
