...
class TSNEAnalyzer(BaseAnalyzer):
    ...
    def validate_params(self, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.config:
                # 从预处理方法参数中获取
                if 'preprocessing_special_params' in self.config and 'tsne' in self.config['preprocessing_special_params']:
                    preprocess_config = self.config['preprocessing_special_params']['tsne']
                ...
                if 'ML_model_special_params' in self.config:
                    ...
                    for task_type in self.config['ML_model_special_params'].values():
                        if isinstance(task_type, dict) and 'tsne' in task_type:
                            tsne_config = task_type['tsne']
                            valid_params.update({p for p in tsne_config.keys() if not p.startswith('key_')})
            
            # 如果配置未加载或没有找到参数，使用硬编码的默认参数列表作为降级方案
            if not valid_params:
                valid_params = {
                    'n_components', 'perplexity', 'early_exaggeration', 'learning_rate',
                    'n_iter', 'n_iter_without_progress', 'min_grad_norm', 'metric',
                    'init', 'verbose', 'random_state', 'method', 'angle', 'n_jobs',
                    'square_distances', 'callbacks', 'callbacks_every_iters'
                }
            
            # 检查所有参数名是否在有效列表中
            for param_name in params:
                if param_name not in valid_params:
                    print(f"警告: t-SNE不支持的参数 '{param_name}'")
            ...
            if 'perplexity' in params:
                if not isinstance(params['perplexity'], (int, float)):
                    raise ValueError("perplexity必须是数字")
                if params['perplexity'] <= 0:
                    raise ValueError("perplexity必须大于0")
            ...
            if 'n_iter' in params:
                if not isinstance(params['n_iter'], int):
                    raise ValueError("n_iter必须是整数")
                if params['n_iter'] < 250:
                    raise ValueError("n_iter必须大于等于250")
            
            if 'angle' in params:
                if not isinstance(params['angle'], float):
                    raise ValueError("angle必须是浮点数")
                if not (0 <= params['angle'] <= 1):
                    raise ValueError("angle必须在[0, 1]范围内")
    ...
    def train(self, X_train: pd.DataFrame, params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        try:
            ...
            if params is None:
                params = {}
            self.model = SklearnTSNE(**params)
            # t-SNE只有fit_transform方法，没有单独的fit和transform方法
            # 在train方法中我们只保存模型配置，实际计算在predict中进行
            return self.model
        except Exception as e:
            raise Exception(f"t-SNE初始化失败: {str(e)}")
    ...
    def save_model_artifacts(self, model: Any, filepath: str) -> bool:
        """
        保存t-SNE模型（t-SNE是不可保存的，因为它不是可转换的模型）
        ...
        返回:
            bool: 是否保存成功
        """
        print("t-SNE模型是不可保存的，因为它不是可转换的模型")
        return False
    ...
    def predict(self, model: Any, X: pd.DataFrame) -> pd.Series:
        ...
        """
        try:
            # t-SNE只有fit_transform方法
            X_reduced = model.fit_transform(X)
            # 返回降维后的数组作为Series
            return pd.Series(X_reduced.flatten())
        except Exception as e:
            raise Exception(f"t-SNE转换失败: {str(e)}")
    ...
    def analyzer(
        ...
        try:
            ...
            # 初始化t-SNE
            initialized_model = self.train(X, params)
            
            # 降维数据
            reduced_data = self.predict(initialized_model, X)
            ...
            if metrics_list or is_return_model_score:
                ...
                # 对于t-SNE，可以计算trustworthiness或continuity等指标
                ...
                if any(metric in calculate_metrics for metric in ["trustworthiness", "continuity"]):
                    try:
                        ...
                        if "trustworthiness" in calculate_metrics:
                            # 计算trustworthiness
                            trust_score = trustworthiness(X, initialized_model.embedding_)
