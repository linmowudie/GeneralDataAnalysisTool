...
class RegressionFactory(BaseFactory):
    """
    回归任务工厂类，负责创建各类回归分析器实例
    """
    
    _regression_analyzers: Dict[str, Type[BaseAnalyzer]] = {}
    ...
    def create_analyzer(self, model_name: str, **kwargs) -> Optional[BaseAnalyzer]:
        ...
        """
        analyzer_class = self._regression_analyzers.get(model_name)
        if analyzer_class:
            try:
                analyzer = analyzer_class(**kwargs)
                print(f"成功创建回归分析器实例: {model_name}")
                return analyzer
            except Exception as e:
                print(f"创建回归分析器实例失败: {model_name}, 错误: {e}")
                return None
        else:
            print(f"未找到对应的回归分析器: {model_name}")
            return None
    ...
    def register_analyzer(self, model_name: str, analyzer_class: Type[BaseAnalyzer]) -> None:
        ...
        """
        self._regression_analyzers[model_name] = analyzer_class
        print(f"已注册回归分析器: {model_name}")
    ...
    def get_available_analyzers(self) -> list:
        ...
        返回:
            list: 可用分析器类型列表
        """
        return list(self._regression_analyzers.keys())