...
class Clustering(BaseAnalyzer):
    ...
    def _compute_scores(self) -> None:
        ...
        selected_metrics = self.metrics_list or list(clustering_metrics.keys())
        self.scores = {}
        ...
        if 'silhouette' in selected_metrics and hasattr(self.trained_model, 'labels_'):
            try:
                if len(np.unique(self.trained_model.labels_)) > 1:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("聚类分析执行完成。")
            return result

        except Exception as e:
            logger.error(f"聚类分析执行失败: {str(e)}")
            raise