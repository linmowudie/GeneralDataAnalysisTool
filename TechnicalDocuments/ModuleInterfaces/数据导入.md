...
- `resource_type` (str): 文件类型（csv/json/xlsx）或数据库类型（sqlite/postgresql/mysql/mssql/oracle/mongodb/redis）
...
df = importer.import_data(query="collection_name")
...
df = importer.import_data(query="key_name")
```
