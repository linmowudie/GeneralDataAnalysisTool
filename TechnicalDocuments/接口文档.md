...
### 1. ManualWorkflow (手动工作流)

这是整个工具的核心类（位于 `backend/Services/workflows/manual_workflow.py`），通过 `execute_step` 统一调度完整的数据分析流程，会话状态由 `WorkflowContext` 维护。
...
##### execute_step(context, step, params)

执行指定步骤并返回产物（Artifact）。步骤顺序：import → preview → cleaning → analysis → visualization → report；import/cleaning/analysis 完成后自动锁定。

**参数:**
- `context` (WorkflowContext): 会话上下文
- `step` (StepName): 步骤枚举（IMPORT/PREVIEW/CLEANING/ANALYSIS/VISUALIZATION/REPORT）
- `params` (Optional[Dict[str, Any]]): 步骤参数字典
...
- `target_col` (Optional[str]): 目标列名，用于字符串目标列自动编码
...
**返回:**
- Artifact: 各步骤产物对象，报告数据可从 REPORT 产物中获取

### 2. 数据导入器 (FileImporter / DbImporter)

负责从文件或数据库导入数据（位于 `backend/Infrastructures/importers`）。通常通过工作流 IMPORT 步骤调用，也可直接使用：
...
# 文件导入（csv/xlsx/json/html，大文件自动分块）
df = FileImporter().read("data.csv")

# 数据库导入（sqlite/mysql/mongodb/redis 等）
df = DbImporter().read(
    "table_name", "sqlite",
...
**说明:**
- `FileImporter.read(source, file_type=None, chunksize=None, **kwargs)`: 按扩展名分发读取
- `DbImporter.read(source, db_type, db_connection_string, query=None, **kwargs)`: 按连接串读取表或 SQL 查询
- 异常统一抛 `ImportError_`，数据库连接由处理器自动关闭
