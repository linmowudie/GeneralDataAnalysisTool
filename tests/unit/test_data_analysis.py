...
class TestDataAnalysis(unittest.TestCase):
    ...
    def test_initialization_with_empty_df(self):
        ...
        with self.assertRaises(ValueError) as context:
            DataAnalyzer(df=empty_df, model="lr")
    ...
    def test_initialization_with_invalid_target_col(self):
        """测试使用无效目标列初始化"""
        with self.assertRaises(ValueError) as context:
            DataAnalyzer(df=self.test_df, model="lr", target_col="invalid_column")
    ...
    def test_initialization_with_invalid_feature_cols(self):
        """测试使用无效特征列初始化"""
        with self.assertRaises(ValueError) as context:
            DataAnalyzer(df=self.test_df, model="lr", feature_cols=["invalid_column"])
    ...
    def test_analyze_method(self):
        ...
        with patch('Src.DataAnalyzer.ModuleInterfaces.data_analysis.AnalyzeData') as mock_analyze_data:
            ...
            mock_analyze_data.return_value = mock_instance
            
            # 创建分析器并执行分析
            analyzer = DataAnalyzer(df=self.test_df, model="lr")
